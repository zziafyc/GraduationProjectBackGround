...
public class ListUtil<T> {
	...
	public List<T> getListNoRepeat(List<T> oldList, String id) {
		if (oldList != null) {
			HashMap<String, T> map = new LinkedHashMap<String, T>();// 换成lingked为了记住之前的顺序
																	// 2016.10.8
			for (T o : oldList) {
	...
	public List<T> sortListDesc(List<T> oldList, final String field) {
		Collections.sort(oldList, new Comparator<T>() {
			...
			public int compare(T o1, T o2) {
				...
				Method m2 = null;
				try {
					className = o1.getClass().getDeclaredField(field).getType().getName();
					m1 = o1.getClass()
							.getMethod("get" + getMethodName(o1.getClass().getDeclaredField(field).getName()));
					m2 = o2.getClass()
							.getMethod("get" + getMethodName(o1.getClass().getDeclaredField(field).getName()));
